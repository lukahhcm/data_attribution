...
autodifferentiation, and (iv) Eq.~(\ref{eq:loss-difference-gradient}). The
...
\begin{enumerate}
    ...
    \item converged finite-penalty VF score, Eq.~(\ref{eq:vf-score});
...
Target and inner batch sizes are separate variables. We first compare target
batches of 32 and 128 examples with the full selected set as a third option.
After fixing that choice, we compare inner batches of 32, 128, and 320 examples
with the full candidate set. Comparisons match examples processed and
separately report optimizer steps. Only the two best configurations are
repeated with all seeds.
