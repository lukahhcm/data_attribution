\section{Expanding the Value-Function--RHO Branch}
\label{sec:connection}

We now develop the third row of
Table~\ref{tab:attribution-correspondence}. This branch is not the paper's only
attribution correspondence; it is the case study through which we examine what
happens after a local score is turned into a repeated selection decision.
...
\end{equation}
For the linear weighting in Eq.~(\ref{eq:fg}), its $i$th coordinate is
\begin{equation}
...
standardization removes only the global scale of Eq.~(\ref{eq:f2sa-gradient}),
...
\begin{equation}
s_{i,\mathrm{RHO}}^{\mathrm{ideal}}(\vomega)
=\ell(\thetahat(\vomega);z_i)
-\ell(\thetatilde(\vomega);z_i),
\label{eq:ideal-rho}
\end{equation}
where the second model incorporates both the holdout objective and the data
represented by $\vomega$. Comparing Eqs.~(\ref{eq:vf-score}) and
(\ref{eq:ideal-rho}) gives
\begin{equation}
\rhoideal(\vomega)
=s^{\mathrm{VF}}(\vomega)
=-\frac{n}{\alpha}
...
Top-$b$ RHO step solves Eq.~(\ref{eq:bilevel-selection}).
...
freezes it, and uses
\begin{equation}
s_{i,\mathrm{RHO}}^{\mathrm{practical}}(t)
...
also means Eq.~(\ref{eq:practical-rho}) is not, in general, the exact gradient
in Eq.~(\ref{eq:rho-vf-equivalence}). The RHO-LOSS paper finds this frozen
