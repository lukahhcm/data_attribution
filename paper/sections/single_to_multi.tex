...
Every identity in Table~\ref{tab:attribution-correspondence} is local: it
supplies a direction at a specified $\vomega$ or along one specified inner
trajectory. We use the VF--RHO correspondence in
Eq.~\ref{eq:rho-vf-equivalence} to study the next question because its
continuous state and practical online endpoint can both be updated explicitly.
Data selection is instead a constrained, path-dependent decision problem. It
must specify an initial state, how long the real target is trained before the
selector changes, whether the same outer state is retained, and how a
continuous state becomes a discrete subset.
